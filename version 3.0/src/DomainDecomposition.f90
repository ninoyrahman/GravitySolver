...
subroutine DomainDecomposition
    ...
    igmin = ngh + coord(0)*iquotient
    jgmin = ngh + coord(1)*jquotient
    kgmin = ngh + coord(2)*kquotient
    ...
    if(.not. allocated(error)) allocate(error(imin:imax,jmin:jmax,kmin:kmax))

    if(.not. allocated(S)) allocate(S(0:ilnc*jlnc*klnc-1, -swidth:swidth,-swidth:swidth,-swidth:swidth))
