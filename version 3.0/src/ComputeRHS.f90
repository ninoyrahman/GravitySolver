...
#include <finclude/petscdef.h>
    ...
    PetscScalar, pointer :: bi(:)
    PetscInt i,j,k
    PetscInt nghi,nghj,nghk
    PetscInt row, low, high
    ...
    do k = kmin, kmax
        do j = jmin, jmax
            do i = imin, imax
                ...
                bi(row+1) = 4.0_rp*pi*G*(ue*rho(i,j,k) + (1.0_rp - ue)*rho4(i,j,k))*h**3
            end do
        end do
    end do
    !$OMP END DO
                    ...
                    do nghj = -swidth, +swidth
                        do nghi = -swidth, +swidth
                            if(IsBoundary(i+nghi,j+nghj,k+nghk) .eq. 1 .and. S(row,nghi,nghj,nghk) .ne. 0.0_rp) then
                                bi(row+1) = bi(row+1) - S(row,nghi,nghj,nghk)*GetBoundaryValue(x(i+nghi),y(j+nghj),z(k+nghk))
                            end if
                        end do
                    end do
                end do
    ...
    call VecRestoreArrayF90(b,bi,ierr)
