 ...
 subroutine CalculateAvgDensity
    ...
    integer :: i,j,k
    real(kind=double) :: Lx,Ly,Lz,Mtmp, Mtmp1
    integer :: ierr
    ...
    !$OMP DO SCHEDULE(STATIC) REDUCTION(+:Mtmp,Mtmp1) COLLAPSE(3)
    do k = kmin, kmax
        do j = jmin, jmax
            do i = imin, imax
    ...
    call MPI_Allreduce(Mtmp, Mtmp, 1, MPI_DOUBLE_PRECISION, MPI_SUM, cart_comm, ierr)
