...
CONTAINS
    ...
    real(kind=double) function GetLogSolution(xcord,ycord,zcord)
        implicit none
        real(kind=double), intent(in) :: xcord,ycord,zcord
        real(kind=double) :: d
        d = sqrt(xcord**2 + ycord**2 + zcord**2)
        GetLogSolution = 4.0_rp*pi*G*1.0_rp*(R**2)*log(d/R)
    end function GetLogSolution

    real(kind=double) function GetExpDecayingSolution(xcord,ycord,zcord)
        implicit none
        real(kind=double), intent(in) :: xcord,ycord,zcord
        GetExpDecayingSolution = (4.0_rp*pi*G/(3.0_rp*R**2))*exp(-R*(abs(xcord) + abs(ycord) + abs(zcord)))
    end function GetExpDecayingSolution
    ...
    real(kind=double) function GetGaussianSolution(xcord,ycord,zcord)
        ...
        real(kind=double) :: d
        d = sqrt(xcord**2 + ycord**2 + zcord**2)
        GetGaussianSolution = -4.0_rp*pi*G*sqrt(pi)*erf(d*8.0_rp)/(4.0_rp*d*512.0_rp)
    ...
    real(kind=double) function GetBoundaryValue(xcord,ycord,zcord)
        ...
        if(scenerio .eq. 0) then
            ...
            GetBoundaryValue =  GetSinSolution(xcord,ycord,zcord) !CalculatePotential(xcord,ycord,zcord)
            ...
            GetBoundaryValue = GetLogSolution(xcord,ycord,zcord)
