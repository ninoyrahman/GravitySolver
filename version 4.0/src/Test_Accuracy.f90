...
#include <finclude/petscdef.h>
    ...
    integer :: i,j,k,row,ierror,usePointValue
    ...
    if(usePointValue .eq. 0) then
        call VecGetArray(phi,phii,offset,ierr)
    else if(usePointValue .eq. 1) then
        call VecDuplicate(phi,pphi,ierr)
        call ComputePointValue(phi,pphi)
        call VecGetArray(pphi,phii,offset,ierr)
    end if
...
!                else
!                    exact = GetR2Solution(x(i),y(j),z(k))
!                    counter = counter + 1
!                    error(i,j,k) = abs(PetscRealPart(phii(offset + 1 + row)) - exact)!/abs(exact)
!                end if
    ...
    if(usePointValue .eq. 1) then
        call VecDestroy(pphi,ierr)
    end if
