...
#include <finclude/petscdef.h>
    ...
    PetscInt row,low,high,stindex
    PetscInt istart, iend, jstart, jend
    real(kind=double) :: u4 = 1.0_rp
    ...
    do k = kmin, kmax
        do j = jmin, jmax
            do i = imin, imax
                ...
                bi(row) = 4.0_rp*pi*G*(u4*rho4(i,j,k) + (1.0_rp - u4)*rho(i,j,k))*h**3
    ...
    if(igmin .eq. ngh) then
        i=imin
        !$OMP PARALLEL PRIVATE(j,k,nghi,nghj,nghk,row,stindex)
                ...
                do nghk = -swidth, +swidth
                    do nghj = -swidth, +swidth
                        do nghi = -swidth, +swidth
                            stindex = stencilindex(nghi,nghj,nghk)
                            if(IsBoundary(i+nghi,j+nghj,k+nghk) .eq. 1 .and. &
                                Stencil(row,stindex) .ne. 0.0_rp) then
                                bi(row) = bi(row) - &
                                    Stencil(row,stindex)*GetBoundaryValue(x(i+nghi),y(j+nghj),z(k+nghk))
                            end if
                        end do
                    end do
                end do
    ...
    if(jgmin .eq. ngh) then
        j=jmin
        !$OMP PARALLEL PRIVATE(i,k,nghi,nghj,nghk,row,stindex)
        !$OMP DO SCHEDULE(STATIC) COLLAPSE(2)
        do k = kmin, kmax
            do i = istart, iend
                row = localindex(i,j,k)
    ...
    if(kgmin .eq. ngh) then
        k=kmin
        !$OMP PARALLEL PRIVATE(i,j,nghi,nghj,nghk,row,stindex)
        !$OMP DO SCHEDULE(STATIC) COLLAPSE(2)
        do j = jstart, jend
            do i = istart, iend
                row = localindex(i,j,k)
