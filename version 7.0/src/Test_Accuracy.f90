...
#include <finclude/petscdef.h>
    ...
    real(kind=double) :: l1, l2, linf, r2, buffer
    PetscScalar, dimension(:), allocatable :: exacti
    PetscInt, dimension(:), allocatable :: rowi
    PetscScalar, dimension(:,:,:), allocatable :: error
    PetscScalar one
    ...
    one = 1.0_rp
    format = '(A9)'
    write (name, format) 'potential'

    call Init
    call Solver(iteration)
    call Output_3D(phi,name,0)
    call Output_1D(ngh+(inc/2),ngh+(jnc/2),ngh+(knc/2))
    ...
    format = '(A5)'
    write (name, format) 'error'
    call Output_3D(err,name,0)

    call MPI_Barrier(cart_comm, ierr)
    buffer = l1
    call MPI_Allreduce(buffer, l1, 1, MPI_DOUBLE_PRECISION, MPI_SUM, cart_comm, ierr)
    buffer = l2
    call MPI_Allreduce(buffer, l2, 1, MPI_DOUBLE_PRECISION, MPI_SUM, cart_comm, ierr)
    buffer = linf
    call MPI_Allreduce(buffer, linf, 1, MPI_DOUBLE_PRECISION, MPI_MAX, cart_comm, ierr)

    l1 = l1/dble(inc*jnc*knc)
    l2 = sqrt(l2**2/dble(inc*jnc*knc))
