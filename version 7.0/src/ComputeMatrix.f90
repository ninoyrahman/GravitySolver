...
#include <finclude/petscdef.h>
    ...
    PetscInt row,lrow,column,stindex
    ...
    !$OMP PARALLEL PRIVATE(i,j,k,nghi,nghj,nghk,row,lrow,column,stindex,proc,whichproc)
                ...
                do nghk = -swidth, +swidth
                    do nghj = -swidth, +swidth
                        do nghi = -swidth, +swidth
                            ...
                            if(IsBoundary(i+nghi,j+nghj,k+nghk) .eq. 0 .and. &
                                Stencil(lrow,stindex) .ne. 0.0_rp) then !S(lrow,nghi,nghj,nghk)

                                icolumn(lrow,column) = globalindex(i+nghi,j+nghj,k+nghk)
                                aa(lrow,column) = Stencil(lrow,stindex) !S(lrow,nghi,nghj,nghk)*576.0_rp/h
