...
CONTAINS
    ...
    real(kind=double) function GetCenteredSolution(xcord,ycord,zcord)
        implicit none
        real(kind=double), intent(in) :: xcord,ycord,zcord
        real(kind=double) :: r2, unirho
        unirho = 1.0_rp/((4.0_rp/3.0_rp)*pi*R**3)
        r2 = (xcord-xc(0))**2 + (ycord-xc(1))**2 + (zcord-xc(2))**2
        if(r2 .le. R*R) GetCenteredSolution = -2.0_rp*pi*G*unirho*(R**2 - (1.0_rp/3.0_rp)*r2)
        if(r2 .gt. R*R) GetCenteredSolution = -(4.0_rp/3.0_rp)*pi*G*unirho*(R**3/sqrt(r2))
    end function GetCenteredSolution
    ...
    real(kind=double) function GetPloytrop1Solution(xcord,ycord,zcord)
        ...
        if(d .gt. R) GetPloytrop1Solution = -4.0_rp*pi*G*(alpha**3)*(pi/d)
    ...
    real(kind=double) function GetBoundaryValue(xcord,ycord,zcord)
        ...
        if(scenerio .eq. 0) then
            ...
            GetBoundaryValue = GetCenteredSolution(xcord,ycord,zcord)
